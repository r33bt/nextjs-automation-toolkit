# Enhanced Next.js Landing Page Setup with Full Vercel Deployment
param(
...
function Load-Credentials {
    ...
    if (Test-Path "$credPath\vercel.token") {
        ...
        Write-Host "❌ Vercel token not found at $credPath\vercel.token" -ForegroundColor Red
        exit 1
    }
    
    if (Test-Path "$credPath\supabase.access.token") {
        $env:SUPABASE_URL = (Get-Content "$credPath\supabase.access.token" -Raw).Trim()
    }
...
$CustomDomain = Get-InputWithDefault "Enter custom domain (optional)" ""
...
# Create .env.local with actual credentials if available
$envContent = @"
...
if ($DeployVercel -eq "y" -or $DeployVercel -eq "Y") {
    ...
    # Deploy to Vercel
    $vercelOutput = vercel --yes --token $env:VERCEL_TOKEN 2>&1
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($env:SUPABASE_URL) {
            ...
            vercel env add NEXT_PUBLIC_SUPABASE_URL production --token $env:VERCEL_TOKEN --value $env:SUPABASE_URL | Out-Null
            vercel env add NEXT_PUBLIC_SUPABASE_ANON_KEY production --token $env:VERCEL_TOKEN --value $env:SUPABASE_ANON_KEY | Out-Null
        ...
        if ($CustomDomain) {
            ...
            vercel alias $vercelUrl $CustomDomain --token $env:VERCEL_TOKEN | Out-Null
            Test-CommandSuccess "Custom domain configured: https://$CustomDomain"
        }
    } else {
...
if ($CreateGitHub -eq "y" -or $CreateGitHub -eq "Y") {
    $githubUrl = "https://github.com/$(gh api user --jq .login)/$ProjectName"
