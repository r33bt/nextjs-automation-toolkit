# Ultimate Next.js Landing Page Setup - Full Automation
param(
...
function Load-Credentials {
    $credPath = "C:\Users\user\alphadev2\_devops\secrets\credentials"
    
    if (Test-Path "$credPath\vercel.token") {
        $env:VERCEL_TOKEN = (Get-Content "$credPath\vercel.token" -Raw).Trim()
        Write-Host "✅ Vercel token loaded" -ForegroundColor Green
    }
    
    if (Test-Path "$credPath\supabase.access.token") {
        $env:SUPABASE_URL = "https://rtbugwhwyqsqcydadqgo.supabase.co"
    }
    if (Test-Path "$credPath\supabase.anon.key") {
        $env:SUPABASE_ANON_KEY = (Get-Content "$credPath\supabase.anon.key" -Raw).Trim()
    }
}

Write-Host "🚀 Ultimate Next.js Landing Page Setup" -ForegroundColor Cyan
...
$DeployVercel = Get-InputWithDefault "Deploy to Vercel automatically? (y/n)" "y"
...
# Step 10: Vercel deployment with full automation
$vercelUrl = ""
if ($DeployVercel -eq "y" -or $DeployVercel -eq "Y") {
    Write-Host "⏳ Deploying to Vercel..." -ForegroundColor Yellow
    
    # Deploy to Vercel with full automation (no prompts)
    $vercelOutput = vercel --prod --yes --token $env:VERCEL_TOKEN 2>&1
    if ($LASTEXITCODE -eq 0) {
        Test-CommandSuccess "Deployed to Vercel"
        
        # Extract URL from output
        $vercelUrl = ($vercelOutput | Select-String "https://.*\.vercel\.app").Matches.Value
        
        # Connect GitHub for auto-deployment
        Write-Host "⏳ Connecting GitHub for auto-deployment..." -ForegroundColor Yellow
        vercel git connect --token $env:VERCEL_TOKEN | Out-Null
        Test-CommandSuccess "GitHub auto-deployment configured"
        
        # Add environment variables to Vercel if available
        if ($env:SUPABASE_URL) {
            Write-Host "⏳ Adding environment variables to Vercel..." -ForegroundColor Yellow
            echo $env:SUPABASE_URL | vercel env add NEXT_PUBLIC_SUPABASE_URL production --token $env:VERCEL_TOKEN | Out-Null
            if ($env:SUPABASE_ANON_KEY) {
                echo $env:SUPABASE_ANON_KEY | vercel env add NEXT_PUBLIC_SUPABASE_ANON_KEY production --token $env:VERCEL_TOKEN | Out-Null
            }
            Test-CommandSuccess "Environment variables added to Vercel"
        }
    } else {
...
# Final success message (NO npm run dev - that was the problem!)
Write-Host ""
Write-Host "🎉 COMPLETE AUTOMATION SUCCESS!" -ForegroundColor Green -BackgroundColor Black
Write-Host ""
...
if ($vercelUrl) {
    Write-Host "Live URL: $vercelUrl" -ForegroundColor Green
}

Write-Host "Local: http://localhost:3000" -ForegroundColor Cyan
Write-Host ""
Write-Host "🚀 To start development server: cd $ProjectName && npm run dev" -ForegroundColor Yellow
Write-Host "✅ Auto-deployment configured: Push to GitHub = Auto-deploy to Vercel" -ForegroundColor Green
